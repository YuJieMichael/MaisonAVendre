...
describe("authentication boundaries", () => {
  ...
  async function waitingSignIn() {
    ...
    sessionStorage.setItem('proprieteavendre.pending-signup', JSON.stringify({email: 'waiting@example.test', at: Date.now()}));
    await render();
  ...
  it('signs in after confirmation in another browser without a pre-existing local session', async () => {
    ...
    expect(sessionStorage.getItem('proprieteavendre.pending-signup')).toBeNull();
  ...
  it.each(['Invalid login credentials', 'Network request failed'])('does not describe %s as an unconfirmed email', async message => {
    ...
    expect(sessionStorage.getItem('proprieteavendre.pending-signup')).not.toContain('test-only-password');
  ...
  it('requires a server session before leaving the waiting screen', async () => {
    ...
    expect(sessionStorage.getItem('proprieteavendre.pending-signup')).toContain('waiting@example.test');
  });
