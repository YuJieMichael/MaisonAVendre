...
type Submission={id:string;property:PublicProperty;contact:{name:string;email:string;phone:string};photo_paths:string[];status:'pending'|'published'|'rejected';revision:number;review_note:string};
...
export function ListingReview({lang}:{lang:Language}){
  ...
  async function refresh(){
    ...
    try{const result=await supabase.from('listing_submissions').select('id,property,contact,photo_paths,status,revision,review_note').neq('status','uploading').order('created_at',{ascending:false}).limit(100);if(result.error)throw result.error;
      ...
      if(active.current){setRows(next);setPhotos(images);}
    }catch{if(active.current){setRows([]);setPhotos({});setError(true);}}finally{if(active.current)setBusy(false);}
  }
