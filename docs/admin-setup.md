...
3. Set Authentication → URL Configuration → Site URL to your production application URL. Add its exact `/#auth/callback` URL to the redirect allowlist. If hosted under a subdirectory, keep that path, for example `https://example.com/MaisonAVendre/#auth/callback`.
...
5. Set the Edge Function secret `APP_ORIGIN` to the application base URL, including its subdirectory when applicable, with no query or fragment. Example: `https://example.com/MaisonAVendre/`. This value determines the allowed CORS origin and invitation redirect; callers cannot choose another redirect.
