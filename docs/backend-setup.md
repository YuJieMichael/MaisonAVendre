...
2. 在免费组织/方案下创建初期测试项目，例如 `proprieteavendre-test`。创建页面如显示付费升级或额外费用，先确认所选方案，不要把本指南理解为购买授权。
...
5. 等待项目建立完成，记录项目设置里的 **Project ID / Reference ID**。也可在项目地址 `https://supabase.com/dashboard/project/PROJECT_REF` 中识别它。`/org/...` 地址内的组织 ID，以及仓库 `supabase/config.toml` 中的本地名称 `proprieteavendre`，都不是云端项目 ID。
...
以 Vite 实际显示的地址为准：`localhost`、`127.0.0.1` 和不同端口需要分别配置。如果网页位于子目录，例如 `/ProprieteAVendre/`，Site URL 与回跳地址都必须保留该路径。正式环境使用明确地址，不配置任意站点通配符。[回跳地址说明](https://supabase.com/docs/guides/auth/redirect-urls)
...
把 `APP_ORIGIN` 换成网页的真实基础 URL。开发测试可用 `http://127.0.0.1:5173/`；部署在子目录时例如 `https://your-domain.example/ProprieteAVendre/`。不要加 `#admin` 或查询参数。它决定邀请邮件回跳和允许的浏览器来源；本实现每个环境配置一个基础 URL。
