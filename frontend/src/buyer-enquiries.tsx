...
export const buyerInboxCopy = {
  en: { title:'Buyer enquiries', hint:'Each request appears individually. Email summaries are sent in batches of 10 when email delivery is configured.', loading:'Loading…', error:'Could not load enquiries. Please retry or verify your administrator access.', empty:'No buyer enquiries yet.', refresh:'Refresh enquiries', previous:'Previous', next:'Next', date:'Submitted', name:'Name', email:'Email', phone:'Phone', city:'Preferred cities', min:'Minimum budget (CAD)', max:'Maximum budget (CAD)', propertyType:'Property type', requirements:'Requirements', timeline:'Timeline', contactLanguage:'Contact language', contactMethod:'Contact method', contactTime:'Contact availability', details:'View request', types:{house:'House',condo:'Condo',plex:'Plex',commercial:'Commercial'} },
...
export function BuyerEnquiries({lang,refreshKey=0}:{lang:Language;refreshKey?:number}) {
  ...
  const load=useCallback(async()=>{
    const version=++generation.current;setLoading(true);setError(false);setRows([]);setMore(false);
    try{
      if(!supabase)throw Error('unconfigured');
      const result=await supabase.rpc('list_buyer_enquiries',{p_offset:offset});
      if(version!==generation.current)return;
      if(result.error)throw result.error;
      const data=(result.data||[]) as Row[];setRows(data.slice(0,50));setMore(data.length>50);
