...
test("invitation URLs preserve the application path and reject unsafe configuration", () => {
  assert.deepEqual(invitationConfig("https://example.com/ProprieteAVendre"), {
    origin: "https://example.com",
    redirectTo: "https://example.com/ProprieteAVendre/#auth/callback",
  });
  assert.equal(
...
function scenario(options: Scenario = {}) {
  ...
  const runtime: Parameters<typeof createInvitationHandler>[0] = {
    env: (name) =>
      ({
        APP_ORIGIN: "https://example.com/ProprieteAVendre/",
