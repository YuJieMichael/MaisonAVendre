...
Deno.serve(async request => {
  ...
  try {
    ...
    const response=await fetch('https://api.resend.com/emails',{
      ...
      body:JSON.stringify({from,to:[RECIPIENT],subject:`ProprieteAVendre — 10 demandes / enquiries — ${batch.id}`,
        ...
        attachments:[{filename:`ProprieteAVendre-${batch.id}.csv`,content}]}),signal:AbortSignal.timeout(15000),
    });
